...
import UtilitiesDemo from 'UtilitiesDemo.json';
...
export default function Home() {
  ...
  const contractAddress = '0x55186a9e6668b59a4fEd9697FB36A78563426fF0'; 
  ...
  const utilitiesDemo = new web3.eth.Contract(UtilitiesDemo.abi as AbiItem[], contractAddress);
  ...
  const handleMint = async () => {
    ...
    utilitiesDemo.methods['mint'](account, 1).send({ from: account, value: value })
    .on("transactionHash", (hash: string) => {
      utilitiesDemo.events.Transfer({}, async (error: any, event: any) => {
  ...
  const handleFetchNFTs = async () => {
    ...
    const account = accounts[0];
    try {
      const balance = await utilitiesDemo.methods['balanceOf'](account).call()
      console.log("balance of ", account, ": ", balance)
      const nftList : NFT[] = [];
      for (let index = 0; index < balance; index++) {
        const {tokenId, tokenUri} = await utilitiesDemo.methods['getOwnerCollectionByIndex'](account, index).call();
        console.log("query result: tokenId: ", tokenId, " tokenUri ", tokenUri)

        // query pending child
        const pendingChildren: Child[] = await utilitiesDemo.methods['pendingChildrenOf'](tokenId).call();
        console.log("child list of tokenId ", tokenId, ":", pendingChildren)

        // query accepted children
        const acceptedChildren: Child[] = await utilitiesDemo.methods['childrenOf'](tokenId).call();
        ...
        const nftInfo: NFT = {
          id: tokenId.toString(),
          name: `Utility-#${tokenId}`,
          description: `this is the number ${tokenId} token of Utility Collection`,
          imageUrl: tokenUri,
  ...
  const fetchPendingChildren = async (parentTokenId: number) => {
    ...
    const account = accounts[0];
    try {
      const pendingChildren: Child[] = await utilitiesDemo.methods['pendingChildrenOf'](parentTokenId).call({from: account});
  ...
  const handleAcceptChildren = async (tokenId: number, childIndex: number, childAddress: string, childTokenId: number) => {
    ...
    try {
      await utilitiesDemo.methods['acceptChild'](tokenId, childIndex, childAddress, childTokenId).send({ from: account })
      .on("transactionHash", (hash: string) => {
        utilitiesDemo.events.ChildAccepted({}, async (error: any, event: any) => {
          console.log("~~~~~~~~~~event: ", event);
